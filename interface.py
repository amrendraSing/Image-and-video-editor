...
def main():
	...
	image = [
		[sg.Radio('Xception Net', 'tone', size=(10,1), key='-XNET-'), sg.Radio('Mobile Net', 'tone', size=(10,1), key='-MNET-')],
	...
	layout_f = [
		[
    		sg.Column(image, size=(800,700), scrollable=True),
    		sg.VSeparator(),
    		sg.Column(layout_right, size=(600,700), scrollable=True) 
    	]
    ]

	# create the window
	window = sg.Window('Photo Editor', layout_f, location=(0, 0))
	...
	while True:
		...
		if event == 'Separate Foreground from Background':
			...
			os.system('python seg.py {} {} {}'.format(inname, outname, str(modeltoggle)))
